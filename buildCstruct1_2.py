...
\n*********************************\n"
    ...
    if options.structure.lower() != 'hopg' and options.structure.lower() !='armcnt'\
        and options.structure.lower() !='zigzagcnt':
...
def add_H(coords,natx,structure,funct_OH):
    ...
    if structure =="hopg" or structure=="armcnt": #saturate hopg in y direction or armchair cnt
        ...
        for i in range(len(coords)-added_atoms-natx,len(coords)-added_atoms): #bottom unsaturated border
            Hx1=Hxz*cos(i*2*pi/natx)
            Hz1=Hxz*sin(i*2*pi/natx)
            Ox1=Oxz*cos(i*2*pi/natx)
            Oz1=Oxz*sin(i*2*pi/natx)
...
def add_NH2(coords, natx, structure):
    '''Add NH2 groups to nonperiodic structures, focusing on armchair carbon nanotubes at both ends.
    '''
    Hcov_r = 0.32 
    Ocov_r = 0.32
    Ccov_r = 0.75
    ...
    if structure =="hopg":  
        ...
        for i in range(len(coords)-natx,len(coords)):
            if modf(float(i)/2.0)[0]==0: #  COO-/COOH is added to every second C 
                ...
                tmpcoords.append(coords[i][2]-ccbond)
                tmpcoords.append(coords[i][3])
                tmpcoords.append('N.2')
                tmpcoords.append(n0charge)
                coords.append(tmpcoords)
                tmpcoords=['H']
                tmpcoords.append(coords[i][1]+Ox1) 
                tmpcoords.append(coords[i][2]-ccbond)
...
def main():
    ...
    if options.structure.lower() == "hopg":
    ...
    elif options.structure.lower() == "armcnt":
